...
if($checkC < 1 && is_numeric($edit)){
  Redirect::to($us_url_root.'users/admin.php?view=forms_edit&err=Form+not+found');
...
if(!empty($_POST['edit_field'])){
  Redirect::to($us_url_root.'users/admin.php?view=forms_edit&edit='.$edit.'&field='.$field);
}
...
if(!empty($_POST['create_field'])){
  ...
  if(!isSqlProtected($col)){
    ...
    Redirect::to($us_url_root."users/admin.php?view=forms_edit&edit=".$edit."&lastOrder=".$lastOrder);
...
if(!empty($_POST['delete_field'])){
  ...
  Redirect::to($us_url_root."users/admin.php?view=forms_edit&err=Field+deleted&edit=".$edit);
}
...
if(!empty($_POST['edit_this_field'])){
  ...
  $db->update($name,$field,$fields);
  Redirect::to($us_url_root."users/admin.php?view=forms_edit&edit=".$edit);
}
